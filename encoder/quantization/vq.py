...
class ResidualVectorQuantizer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, audio_domain, frame_rate: int, bandwidth: tp.Optional[float] = None) -> QuantizedResult:
        ...
        # n_q = self.get_num_quantizers_for_bandwidth(frame_rate, bandwidth) ## n_q 由bandwidth决定
        # n_q = 8
        # nq_choice=[4,8]
        # if self.training:
        #     choice = int(torch.randint(0, 2, (1,)).item())
        #     n_q=nq_choice[choice]
        # breakpoint()
        quantized, codes, commit_loss = self.vq(x, audio_domain, n_q=self.n_q)
        bw = torch.tensor(self.n_q * bw_per_q).to(x)
        return QuantizedResult(quantized, codes, bw, penalty=torch.mean(commit_loss))

    def infer(self, x: torch.Tensor, frame_rate: int, audio_domain, bandwidth: tp.Optional[float] = None) -> QuantizedResult:
        """Residual vector quantization on the given input tensor.
        Args:
            x (torch.Tensor): Input tensor.
            frame_rate (int): Sample rate of the input tensor.
