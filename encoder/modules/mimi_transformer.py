...
from encoder.compile import no_compile
from encoder.modules.gating import make_gating
from encoder.modules.rope import RotaryEmbedding
from encoder.modules.streaming import StreamingModule, StreamingContainer
...
class StreamingTransformerLayer(StreamingModule[_LayerState]):
    ...
    def _ff_block(self, x: torch.Tensor) -> torch.Tensor:
        # breakpoint() # x + norm(linear(act(linear(x))))
        state = self._streaming_state #None
        offset = 0
        if state is not None:
